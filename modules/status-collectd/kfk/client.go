...
// modify return kafka metric format !! terry.zeng
/*
...
// edit by terry.zeng create kafka counter
func (c *Counter) ToKafkaItem4Cnt(falconType string, metricType string, server string) *MItem {
	t, err := time.ParseInLocation("2006-01-02 15:04:05", c.Time, time.Local)
	if err != nil {
		t = time.Now()
	}
	...
	if port != "" {
		tags = "cluster=" + tagName + ",port=" + port
	} else {
		tags = "cluster=" + tagName
	}
...
func (c *Counter) ToKafkaItem(falconType string, metricType string, server string) *MItem {
	t, err := time.ParseInLocation("2006-01-02 15:04:05", c.Time, time.Local)
	if err != nil {
		t = time.Now()
	}
...
func  GetLocalInc(procName *proc.SCount) *MItem {

	endpoint := getHostName()
	metricName := procName.Name
	tags := "cluster=" +  g.Config().Cluster
	value := procName.Cnt
	return &MItem{
		Endpoint: endpoint,
		Metric: metricName,
...
func HealthToKafkaItem(falconType string, server string, health bool) *MItem {
