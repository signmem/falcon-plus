...
func (this *Graph) Query(param cmodel.GraphQueryParam, resp *cmodel.GraphQueryResponse) error {
	...
	lastUpTs := nowTs - nowTs%int64(step)
        pcs := 0
	if step==5 {
	       pcs = rrdtool.RRA5sPointCnt
        }else if step == 10 {
               pcs = rrdtool.RRA10sPointCnt
        }else if step == 30 {
               pcs = rrdtool.RRA30sPointCnt
	}else if step == 60 {
               pcs = rrdtool.RRA1mPointCnt
	}else if step == 300{
               pcs = rrdtool.RRA5mPointCnt
        }else if step == 3600{
               pcs = rrdtool.RRA1hPointCnt
	}else {
               pcs = rrdtool.RRA1mPointCnt
	}

	rra1StartTs := lastUpTs - int64(pcs*step)
	//rra1StartTs := lastUpTs - int64(rrdtool.RRA5sPointCnt*step)
	log.Printf("graph query starttime: %d, rraStartTs: %d, step: %d, pcs: %d \n", start_ts, rra1StartTs,step, pcs)
