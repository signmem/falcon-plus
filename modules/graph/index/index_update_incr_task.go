...
func updateIndexIncr() int {
	...
	if g.Config().Debug {
		log.Printf("[DEBUG] updateIndexIncr() 001 start")
		log.Printf(unIndexedItemCache.String())
		log.Printf("[DEBUG] updateIndexIncr() 001 end")
	}
	...
	for _, key := range keys {
		...
		if icitem != nil {
			// 并发更新mysql
			if g.Config().Debug {
				log.Printf("[DEBUG] updateIndexIncr() 002 start")
				log.Printf("icitem: %v\n",icitem)
				log.Printf("key: %v\n",key)
				log.Printf("[DEBUG] updateIndexIncr() 002 end")
			}
