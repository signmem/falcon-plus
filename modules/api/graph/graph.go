...
func Start(addrs map[string]string) {
	...
	defer func() {
		if r := recover(); r != nil {
			config.Logger.Error("graph got painc")
			config.Logger.Errorf(fmt.Sprintf("%s", r))
			Start(clusterMap)
		}
	}()
	initNodeRings(clusterMap)
	initConnPools(clusterMap)
	config.Logger.Info("graph.Start ok")
}
...
func Delete(params []*cmodel.GraphDeleteParam) {
	...
	for _, para := range params {
		...
		if err != nil {
			config.Logger.Errorf("invalid tags: %s, error: %s", tags_str, err)
			continue
		}
	...
	for pk, node_params := range nodes {
		...
		if err != nil {
			config.Logger.Errorf("select backend node fail, pool:%v, addr:%v, pk:%v, error:%v", pool, addr, pk, err)
			continue
		}

		conn, err := pool.Fetch()
		if err != nil {
			config.Logger.Errorf("fetch conn fail, pool:%v, error:%v", pool, err)
			continue
		}
		...
		if rpcConn.Closed() {
			pool.ForceClose(conn)
			config.Logger.Errorf("conn has been closed, rpcConn:%v", rpcConn)
			continue
		}
		...
		case <-time.After(time.Duration(callTimeout) * time.Millisecond):
			pool.ForceClose(conn)
			config.Logger.Errorf("%s, call timeout. proc: %s", addr, pool.Proc())
		case r := <-ch:
			if r.Err != nil {
				pool.ForceClose(conn)
				config.Logger.Errorf("%s, call failed, err %v. proc: %s", addr, r.Err, pool.Proc())
			} else {
				pool.Release(conn)
			}
			config.Logger.Debugf("Graph.Delete, params:%v, resp:%v", node_params, r.Resp)
		}
	}
}
...
func selectPoolByPK(pk string) (rpool *connp.ConnPool, raddr string, rerr error) {
	...
	if !found {
		config.Logger.Errorf("pool :%v not found", pool)
