...
func ParentScreenGet(c *gin.Context) {
	name := c.Param("screen_name")

	if name == "" {
		h.JSONR(c, badstatus, "invalid screen name")
		return
	}

	screen := m.DashboardScreen{}
	//dt := db.Dashboard.Table("dashboard_screen").Where("name = ?", name).First(&screen)
	dt := db.Dashboard.Table("dashboard_screen").Where("name = ? AND pid = ?", name, "0").First(&screen)
	if dt.Error != nil {
		h.JSONR(c, badstatus, dt.Error)
		return
	}
...
func ScreenGetByIP(c *gin.Context) {
	name := c.Param("screen_ip")

	if name == "" {
		h.JSONR(c, badstatus, "invalid screen IP string")
		return
	}
	...
	ip := func() string {
		...
		ret := reg.FindString(strings.Trim(source, " "))
		if ret != "" {
			return ret
		} else {
			return ""
	...
	if ip == "" {
		h.JSONR(c, badstatus, "invalid screen IP")
		return
	}
...
func ScreenRedirect(c *gin.Context) {
	name := c.Param("screen_name")
	...
	dashboard_url := strings.TrimRight(viper.GetString("dashboard"), "/")
	location := dashboard_url + "/screen/" + strconv.FormatInt(screen.ID, 10) + "?legend=on"
...
func ScreenAll(c *gin.Context) {
	screens := []m.DashboardScreen{}
	dt := db.Dashboard.Table("dashboard_screen").Where("pid <> ?", "0").Find(&screens)
	if dt.Error != nil {
		h.JSONR(c, badstatus, dt.Error)
		return
	}
