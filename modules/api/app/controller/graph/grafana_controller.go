...
func cutEndpointCounterHelp(regexpKey string) (hosts []string, counter string) {
	...
	if len(matchedList) != 0 {
		...
		config.Logger.Errorf("grafana query inputs error: %v", regexpKey)
	}
	return
}
...
func findEndpointIdByEndpointList(hosts []string) []int64 {
	...
	config.Logger.Errorf("[ERROR] hostStr: %v", hostStr)
	config.Logger.Errorf("[ERROR] endpoint in (%s)", hostStr)
...
func GrafanaMainQuery(c *gin.Context) {
	...
	config.Logger.Debugf("got query string: %s", inputs.Query)
...
func GrafanaRender(c *gin.Context) {
	...
	for _, host := range hosts {
		for _, c := range counterArr {
			...
			if err != nil {
				config.Logger.Infof("query graph got error with: %v", inputs)
			} else {
